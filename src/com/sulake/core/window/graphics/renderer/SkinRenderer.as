package com.sulake.core.window.graphics.renderer
{
   ...
   public class SkinRenderer implements ISkinRenderer
   {
      ...
      public function dispose() : void
      {
         ...
         if(!this._disposed)
         {
            for(_loc1_ in this.var_566)
            {
               ISkinLayout(this.var_566[_loc1_]).dispose();
               delete this.var_566[_loc1_];
            }
            this.var_566 = null;
            this.var_469 = null;
            for(_loc1_ in this.var_400)
            {
               ISkinTemplate(this.var_400[_loc1_]).dispose();
      ...
      {
         var _loc3_:ISkinTemplate = this.var_400[param2];
         if(_loc3_ == null)
         {
      ...
      {
         this.var_566[param1.name] = param1;
         return param1;
      }
      ...
      public function removeLayout(param1:ISkinLayout) : ISkinLayout
      {
         var _loc2_:* = null;
         var _loc3_:* = 0;
         param1 = this.var_400[param1.name];
         if(param1 != null)
         {
            for(_loc2_ in this.var_469)
            {
               _loc3_ = uint(_loc2_ as uint);
               if(this.var_469[_loc3_] == param1)
               {
                  this.removeLayoutFromRenderState(_loc3_);
               }
            }
            delete this.var_566[param1.name];
         }
         return param1;
      }
      ...
      public function removeTemplate(param1:ISkinTemplate) : ISkinTemplate
      {
         ...
         if(param1 != null)
         {
            ...
            delete this.var_400[param1.name];
         }
         return param1;
      }
   }
