package com.sulake.habbo.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_478,this.onQueueStatus);
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_478,this.onQueueStatus);
      }
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            this.var_2002 = param1.type;
            this.var_1545 = false;
            this.var_1259 = param1.position;
         }
         else
         {
            this.var_1545 = true;
         }
         this.var_2762 = param1.hasHabboClub;
         this.var_2003 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",this.var_1259.toString());
         localizations.registerParameter("room.queue.position.hc","position",this.var_1259.toString());
         localizations.registerParameter("room.queue.spectator.position","position",this.var_1259.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",this.var_1259.toString());
         this.showInterface();
      }
      ...
      private function showInterface() : void
      {
         ...
         {
            switch(this.var_2002)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_417:
                  _loc1_.caption = !!this.var_2003 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = this.var_1545;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_478:
                  _loc1_.caption = !!this.var_2003 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
      ...
      private function exitQueue(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:RoomWidgetRoomQueueMessage = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_885);
      ...
      {
         messageListener.processWidgetMessage(new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_842));
      }
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(this.var_2002 == RoomWidgetRoomQueueUpdateEvent.const_417)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_934);
         }
         messageListener.processWidgetMessage(_loc2_);
         this.removeWindow();
      }
