package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private const const_2122:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_973 = new Timer(this.const_2122,1);
         this.var_973.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_437)
         {
            this.var_437.addEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_437.addEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_437)
         {
            this.var_437.removeEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_437.removeEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_974);
            if(this.var_1484.indexOf("?") > 0)
            {
               _loc2_ = this.var_1484 + "&retry=" + this.var_974;
            }
            else
            {
               _loc2_ = this.var_1484 + "?retry=" + this.var_974;
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_1865 = true;
         for each(_loc2_ in this.var_1206)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2574,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1207[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1207[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1865)
         {
            this.var_1206.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_535[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_535[_loc3_] = _loc6_;
            }
            this.var_975[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_975)
         {
            _loc7_ = true;
            _loc4_ = this.var_975[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_535[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_975[_loc3_];
         }
         while(_loc6_ < this.var_780.length)
         {
            _loc5_ = this.var_780[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_780.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_973.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_779.length > 0 && this.var_780.length < this.const_1681)
         {
            _loc1_ = this.var_779[0];
            _loc1_.startDownloading();
            this.var_780.push(this.var_779.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_779.indexOf(param1) == -1 && this.var_780.indexOf(param1) == -1)
         {
