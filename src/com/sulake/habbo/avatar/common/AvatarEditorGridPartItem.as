package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_736:IFigurePartSet;
      ...
      private const const_2078:int = 13421772;
      
      private const const_2077:int = 16777215;
      
      private const const_356:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_736 = param3;
         this._window = param1;
         this.var_1374 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         this.var_1726 = param5;
         if(param3 == null)
         {
            this.var_696 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1728 = Math.max(this.var_1728,_loc6_.colorLayerIndex);
            }
         }
         this.var_423 = this.var_69.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_696 != null && !this.var_1726)
            {
               _loc3_ = this.var_696;
            }
            else
         ...
         if(this.isSelected)
         {
            this.var_1374.color = this.const_2078;
         }
         else
         {
            this.var_1374.color = this.const_2077;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_423.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_423.isFigureReady(_loc1_))
         {
            this.var_423.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_356[_loc6_] + "_" + FigureData.const_142;
               _loc4_ = this.var_423.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_356.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_356[_loc6_] + "_" + FigureData.const_142;
                  _loc4_ = this.var_423.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_611 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1727)
               {
                  ...
                  this.var_1727 = _loc9_.content as BitmapData;
               }
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               if(this.var_1726 && _loc2_.colorLayerIndex > 0)
               {
