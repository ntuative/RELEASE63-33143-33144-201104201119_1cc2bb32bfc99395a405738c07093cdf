package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2510 = new RoomThumbnailRenderer(param1);
         this.var_1202 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_190 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_190.visible = true;
         Util.hideChildren(this.var_190);
         this.refreshRoomName(this.var_190,this.var_124);
         if(this.var_124.ownerName != "" && this.var_124.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_190,!!this.var_124.event ? this.getEventCtg(this.var_124.categoryId) : this.getRoomCtg(this.var_124.categoryId));
         this.refreshRoomDesc(this.var_190,this.var_124);
         this.refreshExtraCont();
         this.refreshInfo(this.var_190,"trading_allowed",this.var_124.allowTrading);
         this.refreshInfo(this.var_190,"doormode_doorbell",this.var_124.doorMode == 1);
         this.refreshInfo(this.var_190,"doormode_password",this.var_124.doorMode == 2);
         Util.moveChildrenToColumn(this.var_190,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_190.height = Util.getLowestPoint(this.var_190);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_190,this.var_124.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_190.findChildByName("roomowner"));
         if(this.var_124.ownerName != "" && this.var_124.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_190.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1202.refreshTags(_loc1_,this.var_124.tags);
         this.refreshTextWithCaption("rating",_loc1_,"" + this.var_124.score);
         if(this.var_124.event)
         {
            this.refreshTextWithCaption("startedat",_loc1_,"" + this.var_124.eventCreationTime);
         }
         this.var_2510.refreshThumbnail(_loc1_,this.var_124.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_229,Tabs.const_226,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
