package com.sulake.habbo.tracking
{
   ...
   public class LatencyTracker
   {
      ...
      public function init() : void
      {
         ...
         this.var_1853 = int(this.var_184.getKey("latencytest.interval"));
         this.var_1856 = int(this.var_184.getKey("latencytest.report.index"));
         this.var_2560 = int(this.var_184.getKey("latencytest.report.delta"));
      ...
      public function update(param1:uint, param2:int) : void
      {
         ...
         if(param2 - this.var_1854 > this.var_1853)
         {
            this.testLatency();
         }
      }
      ...
      private function testLatency() : void
      {
         ...
         this.var_644.add(this.var_1478,this.var_1854);
         this._connection.send(new LatencyPingRequestMessageComposer(this.var_1478));
         ++this.var_1478;
      }
      ...
      private function onPingResponse(param1:IMessageEvent) : void
      {
         ...
         var _loc10_:int = 0;
         var _loc11_:* = null;
         if(this.var_644 == null || this.var_229 == null)
         {
            return;
         }
         var _loc2_:LatencyPingResponseMessageParser = (param1 as LatencyPingResponseMessageEvent).getParser();
         var _loc3_:int = this.var_644.getValue(_loc2_.requestId);
         this.var_644.remove(_loc2_.requestId);
         var _loc4_:int = getTimer() - _loc3_;
         this.var_229.push(_loc4_);
         if(this.var_229.length == this.var_1856 && this.var_1856 > 0)
         {
            _loc5_ = 0;
            _loc6_ = 0;
            _loc7_ = 0;
            ...
            if(Math.abs(_loc9_ - this.var_1855) > this.var_2560 || this.var_1855 == 0)
            {
               this.var_1855 = _loc9_;
