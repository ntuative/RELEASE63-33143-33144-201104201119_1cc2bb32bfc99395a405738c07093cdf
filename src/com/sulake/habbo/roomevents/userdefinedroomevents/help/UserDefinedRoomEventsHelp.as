package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private function prepareWindow() : void
      {
         ...
         this._window = IFrameWindow(this.var_81.getXmlWindow("ude_help"));
         this.var_2027 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1271 = this.var_2027.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1271)
         {
            this.var_1271.procedure = this.onButtonClick;
         }
         this.var_1272 = this.var_2027.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1272)
         {
      ...
      {
         if(this.var_277 >= this.var_62.length - 1)
         {
         ...
         if(this.var_277 <= 0)
         {
            this.var_1271.disable();
         }
         else
         {
            this.var_1271.enable();
         }
      }
      ...
      {
         ++this.var_277;
         this.var_277 = Math.min(this.var_277,this.var_62.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_277;
         this.var_277 = Math.max(this.var_277,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         this.var_62 = [];
         var _loc1_:int = 1;
         while(this.var_81.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            _loc2_ = this.var_81.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title");
            _loc3_ = this.var_81.localization.getLocalization("wiredfurni.help." + _loc1_ + ".img");
            _loc4_ = this.var_81.localization.getLocalization("wiredfurni.help." + _loc1_ + ".text");
      ...
      private function localize() : void
      {
         ...
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_62[this.var_277][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_62[this.var_277][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_62[this.var_277][1]).value;
      ...
      {
         if(!param1 || !this.var_81 || !this.var_81.assets)
         {
            return;
         }
         var _loc2_:BitmapDataAsset = this.var_81.assets.getAssetByName(param1) as BitmapDataAsset;
         if(_loc2_ == null)
         {
      ...
      private function retrievePreviewAsset(param1:String) : void
      {
         ...
         var _loc2_:String = this.var_81.configuration.getKey("image.library.catalogue.url");
         var _loc3_:* = _loc2_ + param1 + ".gif";
         Logger.log("[ProductViewCatalogWidget] Retrieve Product Preview Asset: " + _loc3_);
         var _loc4_:URLRequest = new URLRequest(_loc3_);
         var _loc5_:AssetLoaderStruct = this.var_81.assets.loadAssetFromFile(param1,_loc4_,"image/gif");
         if(!_loc5_)
         {
            return;
         }
