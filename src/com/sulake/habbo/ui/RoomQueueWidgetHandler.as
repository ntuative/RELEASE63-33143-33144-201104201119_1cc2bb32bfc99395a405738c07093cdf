package com.sulake.habbo.ui
{
   ...
   public class RoomQueueWidgetHandler implements IRoomWidgetHandler
   {
      ...
      {
         var _loc1_:* = [];
         _loc1_.push(RoomWidgetRoomQueueMessage.const_885);
         _loc1_.push(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         _loc1_.push(RoomWidgetRoomQueueMessage.const_934);
         _loc1_.push(RoomWidgetRoomQueueMessage.const_842);
         return _loc1_;
      }
      ...
      public function processWidgetMessage(param1:RoomWidgetMessage) : RoomWidgetUpdateEvent
      {
         ...
         switch(param1.type)
         {
            case RoomWidgetRoomQueueMessage.const_885:
               this._container.roomSession.quit();
               break;
            case RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE:
               this._container.roomSession.changeQueue(RoomSessionQueueEvent.const_1122);
               break;
            case RoomWidgetRoomQueueMessage.const_934:
               this._container.roomSession.changeQueue(RoomSessionQueueEvent.const_1200);
               break;
            case RoomWidgetRoomQueueMessage.const_842:
               if(this._container.catalog != null)
               {
                  this._container.catalog.openCatalogPage(CatalogPageName.const_206,true);
               }
         }
         return null;
      }
      ...
      {
         return [RoomSessionQueueEvent.const_488];
      }
      ...
      public function processEvent(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            case RoomSessionQueueEvent.const_488:
               ...
               switch(_loc2_.queueSetTarget)
               {
                  case RoomSessionQueueEvent.const_1200:
                     _loc3_ = "null";
                     break;
                  case RoomSessionQueueEvent.const_1122:
                     _loc3_ = "null";
               }
               if(_loc3_ == null)
               {
               ...
               if(_loc5_.length > 1)
               {
                  if(_loc4_ && _loc2_.queueTypes.indexOf(RoomSessionQueueEvent.const_1271) != -1)
                  {
                     _loc6_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_1271) + 1;
                     _loc7_ = true;
                  }
                  else
                  {
                     _loc6_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_1630) + 1;
                  }
               }
               else
               {
